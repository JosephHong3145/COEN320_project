...
// This function essentially links a dummy signal set to the current
// thread so that the SIGALRM signal is handled without the whole
// process crashing. It is called in every thread except the timer thread.
...
void *producerThread(void *data) {
  ...
  // Open the dataset file that was generated by processDataset
  ...
  while(fgets(line, sizeof(line), file) != NULL) {
    ...
    double value = 0;
    // Scan the line and set the shared memory slot
    sscanf(line, "%lf\n", &value);
    *(arguments->sharedMemoryPtr) = value;
  }
  // When the file is done reading, set the shared memory slot to VOI_NUL
  ...
  while(true) {
    ...
    for(int i = 0; i < VOICount; i += 1) {
      if(sharedMemory[i] != VOI_NIL) {
        ...
        // Tally up all of the done threads
        threadsDone++;
      }
    }
    printf("Threads done: %d/%ld\n", threadsDone, VOICount);
    // Exit if all the threads are done
...
void createTimer(uint64_t offset, int period, int identifier) {
  ...
  // Initialises the period and offset of the timer using a specification struct
  ...
  // Sets the timer event to notify the process using a SIGALRM signal with a sigvalue of identifier
  // This is used to decide which semaphore should be unlocked
...
void *timerThread(void *arg) {
  // Configure a signal set to handle
  // the SIGALRM signal for the current thread
  sigemptyset(&sigset);
  ...
  while(1) {
    siginfo_t info;
    // sigwaitinfo instead of sigwait so that the sig_value can be accessed
    sigwaitinfo(&sigset, &info);
    switch(info.si_value.sival_int) {
      case -1:
        // Unlock the consumer semaphore
        sem_post(&consumerWaitSem);
        break;
      default:
        // Unlock one of the producer semaphores
...
int main() {
  ...
  maskThreadFromHandlingAlarmSignal();
  // Initialise semaphores and create the threads
  ...
  for(int i = 0; i < VOICount; i += 1) {
    ...
    // Every single thread is given its own arguments
  ...
  // Wait until all the producer threads and consumer thread is finished
  for(int i = 0; i < VOICount; i += 1) {
